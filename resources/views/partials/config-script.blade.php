...
<script>
window.EDGIFY_CONFIG = {
  ENVIRONMENT_NAME: @json(config('services.edgifynow.environment_name')),
  API_BASE_URL: @json(config('services.edgifynow.api_base_url')),
  APP_BASE_URL: @json(config('services.edgifynow.app_base_url')),
  WIDGET_BASE_URL: @json(config('services.edgifynow.widget_base_url'))
};
</script>
@if(!$envCheck['ok'])
